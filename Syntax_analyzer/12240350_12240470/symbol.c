...
void print()
{
	int i,j;
	printf("\nSYMBOL TABLE\n");
	printf("\nToken\tAddress\tValue\tScope\tType\n");
	for(i=0;i<n;i++)
	{
		if(st[i].type[0]==258)
			printf("%s\t%d\t%d\t%d",st[i].token,st[i].addr,(int)st[i].fvalue,st[i].scope);
		else
			printf("%s\t%d\t%.1f\t%d",st[i].token,st[i].addr,st[i].fvalue,st[i].scope);
		for(j=0;j<st[i].tn;j++)
		{
			if(st[i].type[j]==258)
				printf("\tINT");
